...
//               {isDropdownVisible && (
//                 <div className="absolute top-full -left-28 mt-2 w-[480px] rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 opacity-100 visible transition-all duration-200 transform translate-y-0">
//                   <div className="p-4">
...
//                         <StateIcon key={index} index={index} state={state} />
//                       ))}
//                     </div>
//                   </div>
//                 </div>
...
const StateIcon = React.memo(({ state, index }) => {
  const navigate = useNavigate();
  return (
    <div
      key={index}
...
const SuggestionList = React.memo(({ title, items, itemKey, onSelect }) => {
  if (!items || items.length === 0) return null;
...
const Navbar = () => {
  ...
  const handleSearch = useCallback((suggestion) => {
    navigate(`/search/?query=${encodeURI(suggestion)}`);
    setSearchQuery("");
    setShowDropdown(false);
  }, [navigate]);

  const inputChangeHandler = useCallback((val) => {
    setSearchQuery(val);
    fetchSuggestions(val);
  }, [fetchSuggestions]);
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-lg' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <div
              className="relative group"
              onMouseEnter={() => setIsCategoriesDropdownVisible(true)}
              onMouseLeave={() => setIsCategoriesDropdownVisible(false)}
            >
              <button
                className={`flex items-center space-x-1 ${isScrolled ? 'text-gray-900' : 'text-white'} ${!isScrolled ? 'hover:text-gray-300' : 'hover:text-purple-800'} transition-colors`}
              >
            ...
            <div
              className="relative group"
              onMouseEnter={() => setIsStatesDropdownVisible(true)}
              onMouseLeave={() => setIsStatesDropdownVisible(false)}
            >
              <button
                className={`flex items-center space-x-1 ${isScrolled ? 'text-gray-900' : 'text-white'} ${!isScrolled ? 'hover:text-gray-300' : 'hover:text-purple-800'} transition-colors`}
              >
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden bg-white`}>
        <div className="px-4 pt-2 pb-3 space-y-1">
          {/* Categories */}
          <div className="block px-3 py-2 rounded-md text-gray-700 hover:text-purple-600 hover:bg-purple-50 transition-colors">
            <div className="flex justify-between items-center" onClick={() => navigate('/categories')}>
              <span>Categories</span>
              <ChevronDown className="w-4 h-4" />
            </div>
            <div className="mt-2 space-y-2">
              {categories.map((category, index) => (
                <div
                  key={index}
                  onClick={() => navigate(`/category?name=${encodeURI(category.Nameid)}`)}
                  className="pl-4 py-1 text-sm text-gray-600 hover:text-purple-700"
                >
                  {category.name}
                </div>
              ))}
          ...
          {/* States */}
          <div className="block px-3 py-2 rounded-md text-gray-700 hover:text-purple-600 hover:bg-purple-50 transition-colors">
            <div className="flex justify-between items-center" onClick={() => navigate('/states')}>
              <span>States</span>
              <ChevronDown className="w-4 h-4" />
            </div>
            <div className="mt-2 space-y-2">
